...
**Status:** built, and running. Build order steps 1–10 are done — schema, every Section 9 screen, licence delivery with browser-side signing, new customer through cPanel UAPI, subscriptions, health, removal, the panel's own backup, deploy, and step 10, whose own half was run on 2026-09-05. **Halabja-phone was taken on, under its new name Hamza, on 2026-09-11** — around the database Section 13 insisted on keeping. See Section 11 step 10, which until then recorded that half as dropped.

What is still owed is operational rather than built: **the cron entry that calls `schedule:run` has never existed on this account**, so nothing has ever run itself — see the 2026-09-11 row below.

This line said *“the panel itself is not written yet”* until 2026-09-11, long after it was. A status that has to be re-read against the task log is worse than none.
...
| Run a shop's backup, and download it | Logged. The download takes the log row, never a path — the panel only hands over a file it has a record of writing. |
| Run a shop's migrations | Hold to confirm. Backup taken first, and if the backup fails the migration does not run. Their own `migrate`, through their own artisan. |
| Read a shop's health and data check | Read-only by construction. |
| Throw away what a shop compiled | Logged. Destroys nothing. **The only command the panel runs on a shop for its own sake** — see below. |
...
**⚠️ There is no box to type a command into, and there never will be.** Every
row above is a named thing with a button; the panel builds no artisan command
out of anything an operator typed, and `ClearCompiledCodeTest` fails if that
changes. A panel that runs arbitrary commands on a customer's install is remote
code execution on somebody else's shop, reachable by whoever gets into this
panel — larger than any other hole here, and not something a typed confirmation
makes safe. When a shop needs something the panel cannot do, the answer is a new
row in this table, not a text field.
...
| **Health** | Each shop's own report on itself, read hourly — and, above them, the panel's own backup: when it last ran, whether a copy leaves this machine, and a button to take one now or download the newest. It is on this page because this is where you ask "is anything wrong", and a backup that stopped a fortnight ago is the most wrong thing there can be while every other number is green. |
...
10. **Soran's own shop, created through the panel on the server. Done 2026-09-05.**
    ...
    **That changed on 2026-09-11: they came back, under the name Hamza.** The
    install was rebuilt around the database Section 13 kept, so both halves of this
    step have now been run on the account and the take-on path is no longer a
    screen exercised only by its tests. The line above about no longer working for
    them is left as written rather than corrected — it was true when it was
    written, and a document that quietly edits its own past is one you cannot use
    to work out why a decision was made.
...
| Date | Done | Next |
|---|---|---|
| 2026-09-12 | **The panel could destroy a live shop by removing a different one. Caught before it happened, by Soran asking first.** He wanted to tidy away the old Halabja Phone record and asked whether Hamza's database would go with it. It would have. Taking on a shop builds a new customer around an EXISTING database — that is the whole point, and Section 13 kept that database so it could be done — so what it leaves behind is two rows naming one database: `Halabja Phone` and `New Hamza`, both on `soransto_halabjaphone_shop`. `remove()` runs `drop($customer->database_name, …)`, and `blocked()` asked one question only: is this shop trading? The old row was not, so the button was open and the teardown would have been faultless right up to dropping a live shop's data. **The guard is `sharesSomethingWith()`** — another record naming the same database, `shop_home` or `public_path`, trashed rows included, because a soft-deleted customer's database has not been dropped unless it went through `remove()`. **And a guard alone would have been a dead end**, which is its own hazard: a row that can be neither removed nor tidied away is one somebody eventually DELETEs straight out of the database by hand — the exact "deleted by hand in a hurry" that Section 7 already argued against when it allowed removal at all. So `retire()` is the other half — and **the first version of it destroyed nothing at all, which Soran said plainly was wrong.** Taking on a shop reuses the database and its user and *nothing else*: it takes a new short name, so `paths($short)` gives it new folders, and `refuseIfAnythingIsInTheWay()` guarantees they are nobody's. The old record's subdomain, DNS record and folders therefore belong to it alone, and leaving them is not caution — it is the litter this class was written to stop, and that same litter is what blocks the name being used again. `retire()` is now `remove()` with exactly one step left out, and it names the step: DNS, subdomain and both folders go; the database and its user stay, and the report says which shop they were kept for. No dump is taken, deliberately — `remove()` dumps because it is about to drop the database, and here the database is the one thing that survives. It is offered only when removal is refused and only when the shop is not trading, so it cannot become the quiet way to hide a live customer. Six tests; two go red with the guard removed. **Worth recording as method:** nothing here was found by a test or a tool. It was found because the person about to press the button asked what it would do first. | An edit screen for a customer's safe fields — the panel has none at all | 
| 2026-09-11 | **Halabja-phone is back, under the name Hamza, and the decision to keep its database is what made that possible.** Section 4 deleted that install's folder for serving its `.env` to anyone; Section 13 ruled the database must be kept, "because that is what a rebuilt install restores from"; Section 11 step 10 then recorded the take-on half as *not needed*, since Soran stopped working for them on 2026-09-02. Nine days later they came back and it was needed after all — the install was rebuilt around the kept database, and the take-on screen was run against real data for the first time rather than only by its tests. **Worth keeping as the argument for the rule:** the cheap decision was to drop the database with the folder, and nothing would have survived it. **The finding of the day is cron.** **The one thing that turned out never to have been done is cron.** `panel:check` warned about a backup five days old beside a health check eleven hours old, which does not fit a single cause — and `crontab -l` printed nothing at all. The proof is in the backup filenames: every copy the panel holds is at a human hour, `22:50` and `22:51` a minute apart, none at the `02:30` the schedule asks for. Every backup and every health check this panel has ever taken was run by hand; `schedule:list` showing "Next Due" is the definition, not evidence that anything calls it. DEPLOY.md step 7 is the step that was missed, and until it is added nothing here runs itself: not the hourly check, not the nightly backup. Worth recording as method — the warning that led here was read as two problems and was one absence, and the filenames settled it where the timestamps alone could not. | The cron entry (DEPLOY.md step 7), then `PANEL_BACKUPS_OFFSITE` |
| 2026-09-11 | **Every test the panel has, actually run — 517 of 517, nothing skipped.** It was 488 passing with **29 skipped**, and every skip said the same thing: *this machine has not got the thing*. No MySQL, no `mysqldump`, no `git`, no `posix`, no `public/build`. A test that skips itself in the only place it ever runs is not a test — and among these were the three that matter most here: **the panel's own backup, the restore drill, and taking on a shop**. Section 13 calls losing this database worse than losing any one shop and asks for a drill before go-live; the drill had never been run. It has now, on real MariaDB 10.11, and **it passes**: it fills a scratch database, dumps it, drops the whole database rather than its tables, checks `information_schema` to prove nothing survived, restores and reads the customer back. All 26 MySQL-dependent skips ran and **all 26 passed** — there was nothing hiding in them, which is worth knowing rather than assuming. **CI was the real gap, closed three ways.** It installs `mariadb-client`, because the backup and the drill shell out to `mysqldump` and `mysql` and skip silently without them — a skipped drill is not a drill. It adds `posix`, which is how the panel asks who the web user is. And it **borrows the shop system's compiled `build/` by sparse checkout**, which is the one that would never have been found otherwise: `BorrowedStylesheetTest` checks that the entry names the panel's layouts ask `@vite` for are keys in the *borrowed* manifest — the panel has no `resources/scss/app.scss` and no npm at all — so if the shop system renames an entry, every panel page throws. That check skipped whenever `public/build` was absent, which in CI was always, so **the only guard on a real coupling between two repositories ran nowhere**. Now: SQLite job 491 passing with 26 legitimately-MySQL skips; MariaDB job 517 passing with none. | Step 10 — Soran's own shop, then Halabja-phone |
| 2026-09-11 | **Clearing a shop's compiled code, on purpose.** Section 3 gives every shop its own `bootstrap/cache`, so each holds its own compiled copy of code that is shared and changes underneath it — and the panel has always run `optimize:clear` on every shop as part of updating the shop system. What was missing was doing it when the update is not the reason: an update that half-failed, a file changed on the server, a shop provisioned mid-change. A button on the shop's own page, and one on Updates for all of them at once; the update path now calls the same method rather than keeping a second copy of "clear them all". Nothing stops at the first failure — a shop whose folder has gone must not prevent the others being cleared. **Asked for as "can the panel run commands per shop", and answered as a named button rather than a command box**, which Section 7 now states outright: an operator-typed artisan command running on a customer's install is remote code execution on somebody else's shop, reachable by whoever gets into this panel, and a typed confirmation does not make it safe. A test reads the source and fails if anything ever builds a Process out of request input. Suite: 517 tests, 517 passing, on both engines. | — |
| 2026-09-08 | **Updates could say nothing about the shop system**, because the branch its checkout follows was merged into `main` and GitHub deleted it — the ordinary end of a branch. All the screen showed was git's own sentence, `fatal: couldn't find remote ref claude/shop-provision`, which reads like a broken server and sends somebody looking for damage that is not there. Nothing was wrong: the code was fine and following something that had gone. It is now explained in words, drawn amber rather than red, keeps its branch and commit visible, and carries a button that moves it onto the repository's default branch. **The default branch is asked of the remote** (`ls-remote --symref`) rather than assumed to be `main` — a test renames it to `trunk` and expects the panel to follow. Two guards on the move, and the second is the one that matters: the tree must be clean, and every commit here must already be contained in the default branch. A branch GitHub deleted after merging holds nothing that is not in `main`; a branch deleted by mistake may hold work nobody else has, and the panel cannot tell them apart by name — so it asks git and refuses rather than abandoning the commit. Driven against real git repositories, as the rest of this screen is, because everything worth testing here is git's behaviour. Suite: 507 tests, 507 passing, on both engines. | — |
| 2026-09-06 | **The last two gaps closed, and Section 7's table is now entirely built.** *Run a shop's backup, and download it* — the same dump the panel already took before migrating and before removing, with no second half: the answer to "send me their data" and to "I am about to do something by hand and want a copy first". **The download takes the log row, never a path.** This route hands over a whole customer's database, so nothing the operator types reaches the filesystem: the row is looked up, it must belong to that customer, and the file is whatever the panel recorded writing — held by a test that fails when the ownership check is removed. There is also no folder to whitelist even if that were wise, because the shop system reads `setting('backup_path')` and then `BACKUP_PATH` from the shop's own `.env`. A migration's backup and a removal's are offered the same way and labelled: the second is the only thing left holding a removed shop's data and can never be taken again, so that page reaches it too. And **recovering a password is written down at last** — the one way into the panel that does not need the password left no trace, a TODO waiting on a table that had existed for a week. There is no session at that moment, so `Action::record` grew a narrow `by:` for the single case where the actor is known and not signed in; `accepts()` now returns WHICH way in was used, because spending one of a finite set of recovery codes is worth telling apart from having the phone. Suite: 497 tests, 497 passing, on both engines. | Nothing outstanding in Sections 7, 11 or 13 |
| 2026-09-06 | **Run a shop's migrations** — Section 7 promised it and the button had said "Not built yet" since the danger zone was drawn. **This is Section 3's other half:** one codebase updated once is the whole point, and it leaves every shop's database behind until somebody runs its migrations. `Updater` refuses to do it as a side effect of a code update — "rather than quietly running `migrate` on other people's databases" — and nothing else did it at all, so the Health screen counted shops behind and offered nothing to press. The rule is Section 7's: a backup first, taken with their own `backup:run`, and **if the backup fails the migration does not run** — held by a test that fails when the two are swapped. What runs is the shop's own `migrate` through its own artisan, so Section 7's last line is untouched: the panel still writes nothing to their tables itself. A failure is recorded before it is thrown, because Laravel stops at the migration that threw and keeps the ones before it — "it failed" is never "nothing happened", and the row naming the shop, the count and the backup is what a restore is decided from. A fresh reading is taken and kept afterwards, so the screens stop saying "behind" the moment it is fixed rather than at the next hourly run. Also extracted `App\Support\ShopBackup`: three things now take a shop's backup before touching it, and the "ask where it went, never guess" lesson from this morning lives in one place instead of being reimplemented per caller. Suite: 483 tests, 483 passing, on both engines. | Run a shop's backup and download it — the last "Not built yet" in Section 7 |
| 2026-09-06 | **UAPI has no `SubDomain::delsubdomain` on this account**, and the check added an hour earlier is what said so: "The system could not find the function “delsubdomain” in the module “SubDomain”". `addsubdomain` is in UAPI — creating shops has always worked — and cPanel kept the delete in API2 without moving it across. So `Uapi` grew an `api2()` sibling and `CpanelDomainMaker::remove()` tries both, **asking cPanel after each whether the domain has actually gone** — that answer ends the loop, not either call saying yes, because a function that exists and reports success while leaving the domain in place is what the previous fix was for. The API2 envelope is read generously (its errors appear in different places by version) precisely because the parse is not what decides. Worth recording as method: the panel was made to VERIFY before anyone knew why the domain survived, and the verification then produced the exact cause in one line — where guessing at cPanel's API is what cost a night in Section 11 step 10. Suite: 472 tests, 472 passing, on both engines. | — |
| 2026-09-06 | **A removed shop left its subdomain behind, and the panel said it had not.** `hawler.soranstore.com` stayed in cPanel's Domains list pointing at a folder that no longer existed, after a removal reported as complete — the worst shape a report can have, because everything irreversible had happened and the one part that had not was the part it claimed had. Two causes, both mine. `CpanelDomainMaker::remove()` **trusted cPanel's answer**: `delsubdomain` returning status 1 with no errors is not the same as the domain being gone, so it now asks `DomainInfo::single_domain_data` afterwards — cPanel refuses to describe a domain the account does not have, and that refusal is what "gone" means. And `ShopRemover` **claimed to have removed a domain on a panel that does not point domains at all**: the DNS line said "by hand" where the panel is not the one publishing names, and the domain line did not. Also, what a removal could not finish only ever existed in the flash message — click away and a subdomain pointing at a deleted folder was recoverable only from a JSON blob truncated to 140 characters in the log; it is now on the removed shop's own page, which is where anybody goes back to ask what happened. All three are held by tests that fail against the old code. Suite: 470 tests, 470 passing, on both engines. | — |
| 2026-09-06 | **Removing a shop failed on the first real one**, with "`backup:run` finished without error and left no file in [.../storage/app/backups]" — pointing at a folder that did contain the backup. `BackupService::directory()` appends `daily` or `monthly`, so every dump is one level below where the panel looked. **The test double is why this shipped:** the fake shop in `RemoveShopTest` wrote its dump flat into `backups/`, which encoded my assumption rather than the shop system's behaviour, so nineteen tests agreed with the bug. It now writes where the real one writes and prints what the real one prints, and thirteen of them fail against the old code. The fix is not a deeper search but a different question: `backup:run` prints the path it wrote, so **the shop is asked rather than guessed at** — which also covers the case no amount of searching could, since the folder is `setting('backup_path')` first and `BACKUP_PATH` from the shop's own `.env` after that, and a shop backing up to an external drive was never going to be found under its own folder. The recursive search stays as the fallback for the day that output changes shape. Suite: 466 tests, 466 passing, on both engines. | — |
| 2026-09-06 | **The panel's own backup**, which Section 13 had settled the design of and nothing had built — the last thing here with no copy of it anywhere. `panel:backup` nightly at 02:30, thirty nights and twelve month-ends, an off-machine copy, and `panel:restore`. Section 13's "reuses the shop system's `BackupService`" is corrected above with the reason: 919 lines built around a `Setting` model, an `ActivityLogger` and a storage meter that do not exist here, and a `settings` table the panel has no migration for — depending on another application's internals for the one thing that must work when everything else is broken is the wrong direction. Its *shape* is reused, and its two hard-won details: the password in a 0600 file read through `--defaults-extra-file` rather than on a command line every account on a shared host can read out of `ps`, and `--single-transaction`. **Two things found by driving it rather than by writing it.** The empty-dump guard **could never fire**: it checked `filesize()` of the `.gz`, and gzip writes a header and a footer whatever you give it, so a dump of nothing lands as a valid twenty-byte archive that looks like insurance and restores an empty database — it now counts what mysqldump actually wrote, and the test fails against the old check. And "when did it last run" is read off the files rather than stored, because a stored timestamp with no file beside it is exactly the failure a backup exists to survive. **The drill is the test that matters**: it creates a scratch database, migrates it, puts a customer in it, backs it up, DROPS THE WHOLE DATABASE and restores — checked by breaking the restore and watching it go red, because a passing test that proves nothing is worse than no test. Also on the Health screen: when it last ran, whether anything leaves this machine, **Back up now** and **Download the newest** — which is the off-machine copy in practice, since a file on Soran's laptop survives the server and a second folder on it does not. Suite: 463 tests, 463 passing, on both engines. | Nothing outstanding in Section 13 |
...
- **Backups of the panel's own database** — it holds the customer list, the
  ...
  any one shop. Nightly at 02:30, thirty nights and twelve month-ends kept, an
  off-machine copy, and a restore drill. **The drill was run for the first
  time on 2026-09-11** against real MariaDB and passes: it fills a scratch
  database, dumps it, **drops the whole database** — not the tables, the
  database — checks `information_schema` to prove nothing survived, restores
  from the dump and reads the customer back. CI runs it now.
  **Built 2026-09-06** as
  `App\Services\PanelBackup`, `panel:backup` and `panel:restore`.

  ⚠️ **This said it would reuse the shop system's `BackupService`, and it does
  not.** That class is 919 lines built around the shop system's `Setting` model,
  its `ActivityLogger` and its storage meter — none of which exist here — and it
  writes its status into a `settings` table the panel has no migration for.
  Depending on another application's internals for the one thing that must work
  when everything else is broken is the wrong direction. What is reused is its
  shape, and the two things it learned the hard way: the password goes in a 0600
  file that mysqldump reads through `--defaults-extra-file` (FIRST, or mysqldump
  ignores it) rather than on a command line every account on a shared host can
  read out of `ps`; and `--single-transaction`, so the dump is consistent
  without locking the panel out of itself while it runs.
  ...
  stored timestamp saying last night, with no file beside it, is precisely the
  failure a backup exists to survive — so the folder is the record and it cannot
  disagree with itself.

  **Where it goes matters as much as that it runs.** Beside `~/panel`, never
  inside it: the panel is a git checkout that gets pulled and one day recloned,
  and the backup of its own database must not be what goes with it. `panel:check`
  warns if it is pointed there.
