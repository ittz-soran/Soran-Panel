...
**Status:** built, and running. Build order steps 1–10 are done — schema, every Section 9 screen, licence delivery with browser-side signing, new customer through cPanel UAPI, subscriptions, health, removal, the panel's own backup, deploy, and step 10, whose own half was run on 2026-09-05. **Taking on a shop whose database already existed was run for real on 2026-09-11** — see Section 11 step 10, which until then recorded that half as dropped rather than done.
...
10. **Soran's own shop, created through the panel on the server. Done 2026-09-05.**
    ...
    **That changed on 2026-09-11.** A shop whose database already existed was taken
    on for real, so both halves of this step have now been run on the account —
    the take-on path is no longer a screen that has only ever been exercised by
    its tests.
